...
{foreach from=$items item="filter"}
    ...
    <div class="ty-horizontal-product-filters-dropdown{if $filter.selected_variants || $filter.selected_range} active{/if}">
        <div id="sw_elm_filter_{$filter.filter_id}" class="ty-horizontal-product-filters-dropdown__wrapper cm-combination">{$filter.filter}{if $filter.selected_variants} ({$filter.selected_variants|sizeof}){/if}<a class="cm-ajax cm-ajax-full-render cm-history" href="{$reset_url|fn_url}" data-ca-event="ce.filtersinit" data-ca-target-id="{$ajax_div_ids}"><i class="ty-icon-cancel-circle"></i></a><i class="ty-horizontal-product-filters-dropdown__icon ty-icon-down-micro"></i></div>
