...
class DataKeeper
{
    ...
    public static function backup($params = array())
    {
        ...
        $destination_path = Registry::get('config.dir.cache_misc') . 'tmp/backup/';
        ...
        if (!empty($params['compress'])) {
            ...
            if ($result) {
                fn_rename($destination_path . $pack_name . $ext, Registry::get('config.dir.backups') . $pack_name . $ext);
    ...
    public static function backupFiles($params = array())
    {
        ...
        $destination_path = Registry::get('config.dir.cache_misc') . 'tmp/backup/_files/' . $pack_name;
        ...
        if (!empty($params['fs_compress'])) {
            ...
            $result = fn_compress_files($pack_name . $ext, $pack_name, Registry::get('config.dir.cache_misc') . 'tmp/backup/_files/');
    ...
    public static function restore($filename, $base_path = '')
    {
        ...
        if (in_array($file_ext, array('zip', 'tgz'))) {
            ...
            $extract_path = Registry::get('config.dir.cache_misc') . 'tmp/backup/';
            fn_rm($extract_path);
            ...
            if ($type == 'database') {
                ...
                fn_set_progress('parts', sizeof($files_list) * 2);
                ...
                foreach ($files_list as $file) {
                    ...
                    fn_set_progress('echo', __('check_permissions') . ': ' . $file, true);
                }
                ...
                foreach ($files_list as $file) {
                    ...
                    if ($ext == 'sql' && strpos($file, '/') === false) {
                        // This is a DB dump. Restore it
                        db_import_sql_file($extract_path . $file);
                        continue;
                    }

                    fn_set_progress('echo', __('restore') . ': ' . $file, true);
