...
class RenderManager
{
    ...
    public static function renderBlockContent($block, $params = array())
    {
        ...
        if ($cache_this_block) {
            ...
            $cache_this_block = self::registerBlockCacheIfNeeded($cache_key, $block_schema);
        } else {
     ...
     */
    public static function registerBlockCacheIfNeeded($cache_name, $block_schema)
    {
        ...
        if (!empty($cache_params['callable_handlers'])) {
            foreach ($cache_params['callable_handlers'] as $handler_name => $callable_definition) {
                ...
                if (isset($callable_definition[0]) && is_callable($callable_definition[0])) {
                    ...
                    $generate_additional_level(
                        $handler_name,
                        call_user_func_array($callable_definition[0], $arguments)
                    );
                }
            }
        }
