 ...
 */
function fn_settings_actions_general_secure_storefront(&$new_value, $old_value)
{
    if ($new_value !== 'none') {
        if (!fn_allowed_for('ULTIMATE') || (fn_allowed_for('ULTIMATE') && Registry::get('runtime.company_id'))) {
            ...
            if (empty($content) || $content != 'OK') {
                // Disable https
                Settings::instance()->updateValue('secure_storefront', 'none', 'General');
...
{
    $value = $new_value === 'N' ? 'none' : 'Y';
    fn_settings_actions_general_secure_storefront($value, $old_value);
