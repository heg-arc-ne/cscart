...
function fn_ult_delete_category_pre(&$category_id, $recurse)
{
    ...
    if ($recurse) {
        $categories = db_get_array(
            "SELECT category_id, id_path FROM ?:categories WHERE category_id = ?i OR id_path LIKE ?l",
            $category_id, "$id_path/%"
        );
    } else {
...
SQL;
    ...
    foreach ($categories as $category_data) {
        $products_to_delete = db_get_fields($sql, $category_data['category_id'], $id_path . '/%',
            $category_data['category_id'], $id_path . '/%', $category_data['category_id']);
...
SQL;
    ...
    foreach ($removed_sharing_pairs as $company_id => $product_ids) {
        ...
        $products_options_ids = array_keys(fn_get_product_options($product_ids, DESCR_SL, false, false, false, true));
        empty($products_options_ids) || db_query(
            'DELETE FROM ?:ult_product_option_variants WHERE option_id IN (?a) AND company_id = ?i',
            $products_options_ids,
            $company_id
        );
    }
    ...
    if (!empty($non_shared_products_ids)) {
        ...
        empty($non_shared_product_option_ids) || db_query(
            'DELETE FROM ?:ult_product_option_variants WHERE option_id IN (?a)',
            $products_options_ids
        );
    }
}
...
function fn_check_and_update_product_sharing($product_id)
{
    ...
    if ($count == 1 && $company_id == $product_company_id) {
        ...
        if (!empty($product_option_ids)) {
            db_query('DELETE FROM ?:ult_product_option_variants WHERE option_id IN (?a)', $product_option_ids);
        }
    } else {
        $shared = true;
        ...
        if (!empty($deleted_company_ids)) {
            db_query(
                'DELETE FROM ?:ult_product_descriptions WHERE product_id = ?i AND company_id IN (?a)',
                $product_id, $deleted_company_ids
            );

            db_query(
                'DELETE FROM ?:ult_product_prices WHERE product_id = ?i AND company_id IN (?a)',
                $product_id, $deleted_company_ids
            );

            empty($product_option_ids) || db_query(
                'DELETE FROM ?:ult_product_option_variants WHERE option_id IN (?a) AND company_id IN (?a)',
