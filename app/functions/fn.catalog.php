...
function fn_get_product_features($params = array(), $items_per_page = 0, $lang_code = CART_LANGUAGE)
{
    ...
    if (!empty($params['product_id'])) {
        ...
        if (!empty($params['existent_only'])) {
            $condition .= db_quote(" AND ?:product_features_values.feature_id");
        }
    ...
    $limit = '';
    if (!empty($params['items_per_page'])) {
        $params['total_items'] = db_get_field("SELECT COUNT(*) FROM ?:product_features AS pf $join WHERE 1 $condition $group");
        $limit = db_paginate($params['page'], $params['items_per_page'], $params['total_items']);
    }
    ...
    if (!empty($data)) {
        if ($params['variants'] == true) {
            ...
            if (!empty($feature_ids)) {
                ...
                list($vars) = fn_get_product_feature_variants($_params, 0, $lang_code);
                ...
                if (AREA == 'A' && empty($params['skip_variants_threshould'])) {
                    foreach ($data as $feature_id => $feature_data) {
                        if (!empty($feature_data['variants']) && sizeof($feature_data['variants']) > PRODUCT_FEATURE_VARIANTS_THRESHOLD) {
                            unset($data[$feature_id]['variants']);
...
function fn_get_current_filters($params, $filters, $selected_filters, $area = AREA, $lang_code = CART_LANGUAGE)
{
    ...
    if (!empty($standard_fields)) {
        ...
        foreach ($standard_fields as $filter_id => $filter) {
            ...
            if ($structure['condition_type'] == 'D' || $structure['condition_type'] == 'F') {
            ...
            // Dinamic ranges (price, etc)
                ...
                $range = db_get_row("SELECT MIN($db_field) as min, MAX($db_field) as max FROM ?:$structure[table] ?p WHERE products.status IN ('A') ?p", $fields_join . $join, $where . $fields_where);
            ...
            // Variants (vendors, etc)
                ...
                $field_variant_values[$filter_id]['variants'] = db_get_hash_array(
                    "SELECT $db_field FROM ?:$structure[table] ?p " .
                    "WHERE 1 ?p " .
