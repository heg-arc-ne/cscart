    ...
    if ($mode == 'install') {
        ...
        if (!$result) {
            ...
            if (defined('AJAX_REQUEST')) {
                ...
                Registry::get('ajax')->assign('force_redirection', fn_url('upgrade_center.manage'));
            }
...
if ($mode == 'refresh') {
    ...
    Registry::get('view')->assign('upgrade_packages', $upgrade_packages);

} elseif ($mode == 'package_content' && !empty($_REQUEST['package_id'])) {
    $package_id = $_REQUEST['package_id'];
    $content = UpgradeCenter::instance()->getPackageContent($package_id);

    Registry::get('view')->assign('package_id', $package_id);
    Registry::get('view')->assign('content', $content);

} elseif ($mode == 'ftp_settings') {
    Registry::get('view')->assign('id', $_REQUEST['package_id']);
    Registry::get('view')->assign('type', $_REQUEST['package_type']);
    Registry::get('view')->assign('uc_settings', Settings::instance()->getValues('Upgrade_center'));
}
