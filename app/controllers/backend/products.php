...
if ($mode == 'manage' || $mode == 'p_subscr') {
    ...
    if ($features_search['total_items'] <= PRODUCT_FEATURES_THRESHOLD) {
        ...
        Registry::get('view')->assign('feature_items_too_many', true);
    }
}
//
...
//
    ...
    Registry::get('view')->assign('features_search', $features_search);
...
//
    ...
    if (!empty($selected_fields['data']['features']) && $selected_fields['data']['features'] == 'Y') {
        ...
        Registry::get('view')->assign('all_features_search', $all_features_search);
    }
    ...
    if (!empty($_REQUEST['over'])) {
        Registry::get('view')->assign('over', $_REQUEST['over']);
    }
    ...
    if (!empty($_REQUEST['multiple'])) {
        Registry::get('view')->display('views/products/components/products_m_update_features.tpl');
    } else {
        Registry::get('view')->display('views/products/components/products_update_features.tpl');
    }
    exit;
}
