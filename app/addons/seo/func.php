...
function fn_create_seo_name($object_id, $object_type, $object_name, $index = 0, $dispatch = '', $company_id = '', $lang_code = CART_LANGUAGE, $create_redirect = false, $area = AREA)
{
    ...
    if (fn_check_seo_schema_option($seo_var, 'tree_options')) {
        $path_condition = db_quote(" AND path = ?s", fn_get_seo_parent_path($object_id, $object_type, $company_id));
    }
...
function fn_get_seo_parent_path($object_id, $object_type, $company_id = 0)
{
    ...
    if (!empty($schema[$object_type])) {
        ...
        if (!empty($object_schema['tree'])) {
            return $object_schema['path_function']($object_id, $company_id);
        }
...
/**
 * Updates given SEO name according to its object actual state. If owner object doesn't exist, SEO-name will be deleted.
 *
...
function fn_regenerate_seo_name(array $seo_name)
{
    if (fn_check_seo_object_exists($seo_name['object_id'], $seo_name['type'], $seo_name['company_id']) !== false) {
        fn_create_seo_name($seo_name['object_id'], $seo_name['type'], $seo_name['name'], 0, '',
            $seo_name['company_id'], $seo_name['lang_code'], true);
