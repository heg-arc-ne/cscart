...
<tr class="cm-row-status-{$page.status|lower}" {if $page.level > 0 && $search.get_tree == "multi_level"}class="multiple-table-row"{/if}>
    ...
    <td class="row-status" width="70%">
        {strip}
        <div class="text-over" {if $search.get_tree == "multi_level"}style="padding-left: {math equation="x*14" x=$page.level|default:0}px;"{/if}>
    ...
    <td width="10%">
        ...
        {capture name="tools_list"}
            ...
            {if "ULTIMATE"|fn_allowed_for && $allow_save || !"ULTIMATE"|fn_allowed_for}
                <li>{btn type="list" text=__("delete") class="cm-confirm cm-post" href="pages.delete?page_type=`$page.page_type`&page_id=`$page.page_id``$multi_level`&come_from=`$_come_from`"}</li>
            {/if}
        {/capture}
